<template>
    <v-container fluid>
                                            ...
                                            <v-form ref="form">
                                                <v-row align="center" justify="start">
                                                    ...
                                                    <v-btn color="primary" class="ma-6" :disabled="!file"
                                                        @click="cargar()">
                                                        {{ $t("reemplazarDatos.subirArchivo") }}
                                                    </v-btn>
                                                </v-row>
                                            </v-form>
                                        </v-col>
        ...
        <v-card elevation="0" v-if="preData">
            <v-card-title>
                <v-row align="center" justify="end">
                    <v-col cols="6">
                        <v-row align="center" justify="start">
                            <v-btn color="success" elevation="10" class="ma-6" @click="reemplazarDatos()">
                                {{ $t("reemplazarDatos.publicarCambios") }}
                            </v-btn>
                        </v-row>
                    </v-col>
                    <v-col cols="6">
        ...
        <ConfirmDialogVue typeConfirm="reemplazar" :fileNameBackEnd="fileName" />
    </v-container>
</template>
            ...
            let api = Axios.post(Enviroment.url + '/datasets/verify', formData,
                {
                    ...
                    if (response.data.code == 100) {
                        ...
                        this.newMessage("success", this.$i18n.t("reemplazarDatos.archivoCargadoExito"));
                    } else {
                        this.newMessage("error", this.$i18n.t("reemplazarDatos.archivoCargadoError"));
                    }
                }).catch(error => {
                    this.loader = false
                    console.error(error)
